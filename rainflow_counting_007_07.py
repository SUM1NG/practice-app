...
def draw_frequency_distribution(result_dict):
    fig, axs = plt.subplots(len(result_dict), 1, figsize=(10, 6*len(result_dict)))
...
def draw_3d_histogram(result_dict):
    fig = plt.figure(figsize=(10, 6*len(result_dict)))
    for i, (y_name, cycles) in enumerate(result_dict.items(), 1):
        ...
        # 3D 그래프 그리기
        ax = fig.add_subplot(len(result_dict), 1, i, projection='3d')
